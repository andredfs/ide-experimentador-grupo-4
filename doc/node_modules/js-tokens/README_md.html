...
<div class="nav-section">
  ...
  <ul class="link-list" role="directory">
              <li>      <a href="#label-Overview+rdoc-image-3Ahttps-3A-2F-2Ftravis-ci.org-2Flydell-2Fjs-tokens.svg-3Fbranch-3Dmaster">Overview <a href="https://travis-ci.org/lydell/js-tokens"><img src="https://travis-ci.org/lydell/js-tokens.svg?branch=master"></a></a>
          ...
          <li>
            <details open>
              ...
              <ul class="link-list" role="directory">
                          <li>      <a href="#label-jsTokens"><code>jsTokens</code></a>
          <li>      <a href="#label-var+token+-3D+matchToToken-28match-29"><code>var token = matchToToken(match)</code></a>

              </ul>
            </details>
          </li>
          <li>      <a href="#label-ECMAScript+support">ECMAScript support</a>
          <li>      <a href="#label-Invalid+code+handling">Invalid code handling</a>
          <li>
            <details open>
              <summary>      <a href="#label-Limitations">Limitations</a>
</summary>
              <ul class="link-list" role="directory">
                          <li>      <a href="#label-Template+string+interpolation">Template string interpolation</a>
          <li>      <a href="#label-Division+and+regex+literals+collision">Division and regex literals collision</a>
          <li>      <a href="#label-ES2018">ES2018</a>
...
<main role="main" aria-label="Page node_modules/js-tokens/README.md">

<h1 id="label-Overview+rdoc-image-3Ahttps-3A-2F-2Ftravis-ci.org-2Flydell-2Fjs-tokens.svg-3Fbranch-3Dmaster">Overview <a href="https://travis-ci.org/lydell/js-tokens"><img src="https://travis-ci.org/lydell/js-tokens.svg?branch=master"></a><span><a href="#label-Overview+rdoc-image-3Ahttps-3A-2F-2Ftravis-ci.org-2Flydell-2Fjs-tokens.svg-3Fbranch-3Dmaster">&para;</a> <a href="#top">&uarr;</a></span></h1>

<p>A regex that tokenizes JavaScript.</p>

<pre>var jsTokens = require(&quot;js-tokens&quot;).default
...
// or:
var jsTokens = require(&quot;js-tokens&quot;).default</pre>

<h1 id="label-Usage">Usage<span><a href="#label-Usage">&para;</a> <a href="#top">&uarr;</a></span></h1>

<h3 id="label-jsTokens"><code>jsTokens</code><span><a href="#label-jsTokens">&para;</a> <a href="#top">&uarr;</a></span></h3>
...
<h3 id="label-var+token+-3D+matchToToken-28match-29"><code>var token = matchToToken(match)</code><span><a href="#label-var+token+-3D+matchToToken-28match-29">&para;</a> <a href="#top">&uarr;</a></span></h3>

<pre>import {matchToToken} from &quot;js-tokens&quot;
// or:
var matchToToken = require(&quot;js-tokens&quot;).matchToToken</pre>

<p>Takes a <code>match</code> returned by <code>jsTokens.exec(string)</code>, and returns a <code>{type: String, value: String}</code> object. The following types are available:</p>
<ul><li>
<p>string</p>
</li><li>
<p>comment</p>
...
<p>Multi-line comments and strings also have a <code>closed</code> property indicating if the token was closed or not (see below).</p>

<p>Comments and strings both come in several flavors. To distinguish them, check if the token starts with <code>//</code>, <code>/*</code>, <code>&#39;</code>, <code>&quot;</code> or <code>`</code>.</p>
...
<h1 id="label-ECMAScript+support">ECMAScript support<span><a href="#label-ECMAScript+support">&para;</a> <a href="#top">&uarr;</a></span></h1>
...
<h1 id="label-Invalid+code+handling">Invalid code handling<span><a href="#label-Invalid+code+handling">&para;</a> <a href="#top">&uarr;</a></span></h1>
...
<p>Invalid non-ASCII characters are treated as names, to simplify the matching of names (except unicode spaces which are treated as whitespace). Note: See also the <a href="#es2018">ES2018</a> section.</p>
...
<h1 id="label-Limitations">Limitations<span><a href="#label-Limitations">&para;</a> <a href="#top">&uarr;</a></span></h1>

<p>Tokenizing JavaScript using regexes—in fact, <em>one single regex</em>—won’t be perfect. But that’s not the point either.</p>

<p>You may compare jsTokens with <a href="http://esprima.org/">esprima</a> by using <code>esprima-compare.js</code>. See <code>npm run esprima-compare</code>!</p>

<h3 id="label-Template+string+interpolation">Template string interpolation<span><a href="#label-Template+string+interpolation">&para;</a> <a href="#top">&uarr;</a></span></h3>

<p>Template strings are matched as single tokens, from the starting <code>`</code> to the ending <code>`</code>, including interpolations (whose tokens are not matched individually).</p>

<p>Matching template string interpolations requires recursive balancing of <code>{</code> and <code>}</code>—something that JavaScript regexes cannot do. Only one level of nesting is supported.</p>

<h3 id="label-Division+and+regex+literals+collision">Division and regex literals collision<span><a href="#label-Division+and+regex+literals+collision">&para;</a> <a href="#top">&uarr;</a></span></h3>
...
<p>A human can easily understand that in the <code>number</code> line we’re dealing with division, and in the <code>regex</code> line we’re dealing with a regex literal. How come? Because humans can look at the whole code to put the <code>/</code> characters in context. A JavaScript regex cannot. It only sees forwards. (Well, ES2018 regexes can also look backwards. See the <a href="#es2018">ES2018</a> section).</p>

<p>When the <code>jsTokens</code> regex scans throught the above, it will see the following at the end of both the <code>number</code> and <code>regex</code> rows:</p>

<pre>/ 2/g</pre>

<p>It is then impossible to know if that is a regex literal, or part of an expression dealing with division.</p>
...
<p>The first line divides the <code>foo</code> variable with <code>2/g</code>. The second line calls the <code>foo</code> function with the regex literal <code>/= 2/g</code>. Again, since <code>jsTokens</code> only sees forwards, it cannot tell the two cases apart.</p>
...
<pre class="ruby"><span class="ruby-identifier">var</span> <span class="ruby-identifier">foo</span> = <span class="ruby-value">2</span><span class="ruby-operator">/</span><span class="ruby-identifier">g</span>
<span class="ruby-identifier">foo</span> <span class="ruby-operator">/=</span> <span class="ruby-value">2</span>
</pre>

<p>Regex literals cannot contain newlines, so the above cases are correctly identified as division. Things are only problematic when there are more than one non-comment slash in a single line.</p>

<p>Secondly, not every character is a valid regex flag.</p>

<pre class="ruby"><span class="ruby-identifier">var</span> <span class="ruby-identifier">number</span> = <span class="ruby-identifier">bar</span> <span class="ruby-operator">/</span> <span class="ruby-value">2</span><span class="ruby-operator">/</span><span class="ruby-identifier">e</span>
</pre>

<p>The above example is also correctly identified as division, because <code>e</code> is not a valid regex flag. I initially wanted to future-proof by allowing <code>[a-zA-Z]*</code> (any letter) as flags, but it is not worth it since it increases the amount of ambigous cases. So only the standard <code>g</code>, <code>m</code>, <code>i</code>, <code>y</code> and <code>u</code> flags are allowed. This means that the above example will be identified as division as long as you don’t rename the <code>e</code> variable to some permutation of <code>gmiyus</code> 1 to 6 characters long.</p>

<p>Lastly, we can look <em>forward</em> for information.</p>
<ul><li>
<p>If the token following what looks like a regex literal is not valid after a regex literal, but is valid in a division expression, then the regex literal is treated as division instead. For example, a flagless regex cannot be followed by a string, number or name, but all of those three can be the denominator of a division.</p>
</li><li>
<p>Generally, if what looks like a regex literal is followed by an operator, the regex literal is treated as division instead. This is because regexes are seldomly used with operators (such as <code>+</code>, <code>*</code>, <code>&amp;&amp;</code> and <code>==</code>), but division could likely be part of such an expression.</p>
</li></ul>
...
<h3 id="label-ES2018">ES2018<span><a href="#label-ES2018">&para;</a> <a href="#top">&uarr;</a></span></h3>

<p>ES2018 added some nice regex improvements to the language.</p>
<ul><li>
<p><a href="http://2ality.com/2017/07/regexp-unicode-property-escapes.html">Unicode property escapes</a> should allow telling names and invalid non-ASCII characters apart without blowing up the regex size.</p>
</li><li>
<p><a href="http://2ality.com/2017/05/regexp-lookbehind-assertions.html">Lookbehind assertions</a> should allow matching telling division and regex literals apart in more cases.</p>
</li><li>
<p><a href="http://2ality.com/2017/05/regexp-named-capture-groups.html">Named capture groups</a> might simplify some things.</p>
</li></ul>
